...
public class PHP_APIMySQL {
	...
	private void updateRoutes() {
		try {
			...
			routes.append("<?php\n\n")
					...
					.append("use Slim\\App;\n")
					.append("use Slim\\Routing\\RouteCollectorProxy;\n")
					...
					.append("    });\n\n")
					.append("    // API v1 Routes\n")
					.append("    $app->group('/api/v1', function (RouteCollectorProxy $group) {\n")
					.append("        // Auth routes\n")
					.append("        $group->group('/auth', function (RouteCollectorProxy $group) {\n")
					.append("            $group->post('/register', \\App\\Controllers\\AuthController::class . ':register');\n")
					.append("            $group->post('/login', \\App\\Controllers\\AuthController::class . ':login');\n")
					.append("        });\n\n");

			// Add protected auth route if auth table is specified
			if (authTable != null && !authTable.isEmpty()) {
				routes.append("        // Protected auth routes\n")
						.append("        $group->group('/").append(toCamelCase(authTable)).append("', function (RouteCollectorProxy $group) {\n")
						.append("            $group->get('/me', \\App\\Controllers\\")
						.append(toModelName(authTable, true)).append("Controller::class . ':get')")
						.append("->add(AuthMiddleware::class);\n")
						.append("        });\n\n");
			}
			...
			for (Map.Entry<String, FinalQueryData> entry : finalDataDic.entrySet()) {
				...
				String pluralModelNameCamel = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, 
						toModelName(tableName, false));

				routes.append("        // ").append(singularModelName).append(" routes\n")
						.append("        $group->group('/").append(pluralModelNameCamel)
						.append("', function (RouteCollectorProxy $group) {\n")
						.append("            $group->get('', \\App\\Controllers\\")
						.append(singularModelName).append("Controller::class . ':getAll');\n")
						.append("            $group->get('/{")
						.append(entry.getValue().getPrimaryKeys().get(0).getFieldName())
						.append("}', \\App\\Controllers\\")
						.append(singularModelName).append("Controller::class . ':get');\n")
						.append("            $group->post('', \\App\\Controllers\\")
	...
	private void createPHPControllerFile() {
		for (Map.Entry<String, FinalQueryData> item : finalDataDic.entrySet()) {
			try {
				...
				classContent.append("    }\n\n");
				classContent.append("    public function update(Request $request, Response $response, $args): Response\n");
				classContent.append("    {\n");
				classContent.append("        $").append(item.getValue().getPrimaryKeys().get(0).getFieldName())
						.append(" = $args['").append(item.getValue().getPrimaryKeys().get(0).getFieldName())
						.append("'];\n");
				...
				classContent.append(
						"        return $response->withHeader('Content-Type', 'application/json')->withStatus(200);\n");
				classContent.append("    }\n\n");
				classContent.append("    public function delete(Request $request, Response $response, $args): Response\n");
				classContent.append("    {\n");
				classContent.append("        $").append(item.getValue().getPrimaryKeys().get(0).getFieldName())
						.append(" = $args['").append(item.getValue().getPrimaryKeys().get(0).getFieldName())
						.append("'];\n");
	...
	private void createIndexFile(String authTable) {
		try {
			...
			indexContent.append("        'name' => $_ENV['APP_NAME'] ?? 'MyApp',\n");
			indexContent.append("        'env' => $_ENV['APP_ENV'] ?? 'local',\n");
			indexContent.append("        'debug' => $_ENV['APP_DEBUG'] ?? false,\n");
			indexContent.append("        'url' => $_ENV['APP_URL'] ?? 'http://localhost',\n");
			...
			indexContent.append("            'driver' => $_ENV['DB_DRIVER'] ?? 'mysql',\n");
			indexContent.append("            'host' => $_ENV['DB_HOST'] ?? 'localhost',\n");
			indexContent.append("            'port' => $_ENV['DB_PORT'] ?? 3306,\n");
			indexContent.append("            'database' => $_ENV['DB_DATABASE'] ?? 'database_name',\n");
			indexContent.append("            'username' => $_ENV['DB_USERNAME'] ?? 'root',\n");
			indexContent.append("            'password' => $_ENV['DB_PASSWORD'] ?? '',\n");
			indexContent.append("            'charset' => $_ENV['DB_CHARSET'] ?? 'utf8mb4',\n");
	...
	private void createAuthServiceFile(String authTable, String authUsernameField, String authPasswordField) {
		try {
			...
			String modelName = toModelName(authTable, true);
			...
			classContent.append("    {\n");
			classContent.append("        try {\n");
			classContent.append("            $user = new ").append(modelName).append("();\n");
			...
			classContent.append("    {\n");
			classContent.append("        try {\n");
			classContent.append("            $user = ").append(modelName).append("::where('").append(authUsernameField)
			...
			classContent.append("        }\n");
			classContent.append("    }\n\n");
			classContent.append("    private function generateToken(").append(modelName).append(" $user)\n")
					.append("    {\n")
					.append("        $payload = [\n")
					.append("            'user_id' => $user->id,\n")
					.append("            '").append(authUsernameField).append("' => $user->")
					.append(authUsernameField).append(",\n")
					.append("            'exp' => time() + (60 * 60 * 24), // Token expires in 24 hours\n")
					.append("        ];\n\n")
					.append("        $secretKey = $this->container->get('config')['jwt'];\n")
					.append("        return JWT::encode($payload, $secretKey, 'HS256');\n")
					.append("    }\n");
			classContent.append("}\n");
